...
def plot_correlation_matrix(dataframe, **kwargs):
    """
    Useful references:
...
def discrete_colorscale(boundaries, colors):
    ...
    * https://chart-studio.plotly.com/~empet/15229/heatmap-with-a-discrete-colorscale/#/
    ...
    nvals = [(v - boundaries[0]) / (boundaries[-1] - boundaries[0]) for v in
             boundaries]  # normalized values
...
def format_tick_values(boundaries):
    return [np.mean(boundaries[k:k + 2]) for k in
            range(len(boundaries) - 1)]


def format_tick_text(boundaries):
    return [f'<{boundaries[1]}'] + [f'{boundaries[k]}-{boundaries[k + 1]}' for k in
                                    range(1, len(boundaries) - 2)] + [f'>{boundaries[-2]}']
